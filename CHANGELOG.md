...
- 修复 create 向导中超长输入导致 pi 崩溃的问题：输入行不再无上限渲染，
  改为尾窗显示（光标始终在末尾，超宽部分以 `…` 截去头部），超宽输入
  永远不会撑破终端宽度
- name / description 输入改为硬上限（64 / 1024 字符）：达到上限后继续输入
  被忽略（强制截断），并显示 `limit reached` 提示；计数器常显 `(n/上限)`
- 面板的过滤输入同步加固：超长过滤词同样尾窗显示，不再可能产生超宽渲染行
