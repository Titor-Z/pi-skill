...
- **面板交互重设计**（对齐内置 `/scoped-models` 的体验）：空格切换即时保存，不再需要回车确认；
  Esc/q 随时关闭，无"未保存"状态
- 新增统计行（`N skills · X enabled · Y disabled`）与 `/` 实时搜索过滤
- 列表支持滚动 viewport，长列表不再截断
- 检测 settings 中指向已不存在 skill 的残留 `!条目`，提供 `c` 一键清理
