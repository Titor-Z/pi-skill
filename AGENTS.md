...
### 2026-09-08 会话 6 — 面板细节打磨：边框、常显统计、详情折行

- 用户反馈三项：① 面板应有上下边框和颜色（对齐 scoped-models 的 DynamicBorder）；② 底部提示栏距左侧不应有空格；③ 详情行应允许换行，续行与首行保持相同左 padding。
- 实现：上下边框用主题 `border` 色（`theme.fg("border", "─".repeat(width))`，与 pi 内置 DynamicBorder 同源）；页脚去缩进；`(n/N)` 改为常显（单条过滤结果也显示 `(1/1)`）；详情行改为按宽度 word-wrap（`wrapDetail`，续行同缩进），错误信息同样折行，不再截断加省略号。
- 冒烟测试同步更新（边框存在、页脚无缩进、折行续行对齐、`(1/1)` 常显），隔离 HOME 复跑 ALL PASS。

---

### 2026-09-08 会话 6 续 — 详情列表化排版

- 用户要求：TUI 下描述里的枚举（有序/无序）应按列表逐行排列，否则阅读排版差。
- 实现（渲染层，不改 SKILL.md——description 规范上必须是单行字符串）：`splitList` 用前瞻正则识别行内枚举标记（`-`/`•`/`*`、`\d{1,2}[.)]`、`\([a-z0-9]{1,2}\)`、`[a-z]\\)`，需 ≥2 个才判定为列表，避免 e.g. 误判）；intro 段落照常折行，每个条目独立成块（缩进 4），条目续行与条目文本（marker 之后）对齐。
- 测试教训：fixture 用模板串生成时，字符串字面量替换会静默失配——本次 `Filler number ${i}` 差点漏改，靠断言失败回查；另外模糊过滤用例的查询词会被新增 fixture 污染唯一性（"beta" 子序列意外命中新描述），改用数字唯一查询 "05"。
- 冒烟测试扩到 52 项，ALL PASS。
